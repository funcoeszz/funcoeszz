...
zzmat()
{
	...
	if zztool grep_var '^' "$funcao" && zztestar numero_real "${funcao%^*}" && zztestar numero_real "${funcao#*^}"; then
		zzmat -p${precisao} pow "${funcao%^*}" "${funcao#*^}"
		return
	elif zztool grep_var '!' "$funcao" && zztestar numero "${funcao%\!}"; then
	...
	case "$funcao" in
		sem_zeros)
			...
			;;
		compara_num)
			if test $# -eq "3" && zztestar numero_real $2 && zztestar numero_real $3; then
				local num1 num2
				num1=$(echo "$2" | tr ',' '.')
				num2=$(echo "$3" | tr ',' '.')
				echo "$num1 $num2" |
		...
		int)
			...
			shift
			num1=$(zztool multi_stdin "$@" | tr ',' '.')
			if zztestar numero_real $num1; then
				echo $num1 | sed 's/\..*$//'
			fi
			;;
		abs)
		...
		sen | cos | tan | csc | sec | cot)
			if test $# -eq "2"; then
				...
				if test "$?" -eq "0" && zztestar numero_real $num1; then
					case $ang in
			...
			if test $# -ge "2" && test $# -le "4" && zztestar numero_real $2; then
				...
				case $funcao in
					...
					asen)
						...
						if test "$num3" = $(printf '%.'${precisao}'f' 0 | tr ',' '.'); then
							num2=$(echo "$pi/2" | bc -l)
						else
	...
	atan: Arco-Tangente"
			...
			;;
		log | ln)
			if test $# -ge "2" && test $# -le "3" && zztestar numero_real $2; then
				local num1 num2
				num1=$(echo "$2" | tr ',' '.')
				zztestar numero_real "$3" && num2=$(echo "$3" | tr ',' '.')
				if test -n "$num2"; then
					num="l($num1)/l($num2)"
				elif test "$funcao" = "log"; then
					num="l($num1)/l(10)"
				else
					num="l($num1)"
				fi
			...
			;;
		raiz)
			if test $# -eq "3" && zztestar numero_real "$3"; then
				local num1 num2
				case "$2" in
					quadrada) num1=2 ;;
					c[úu]bica) num1=3 ;;
				...
				num2=$(echo "$3" | tr ',' '.')
				if test $(($num1 % 2)) -eq 0; then
					if echo "$num2" | grep '^-' >/dev/null; then
		...
		area)
			if test $# -ge "2"; then
				...
				case "$2" in
					triangulo)
						if zztestar numero_real $3 && zztestar numero_real $4; then
							num1=$(echo "$3" | tr ',' '.')
							num2=$(echo "$4" | tr ',' '.')
							num="${num1}*${num2}/2"
						else
							zztool erro " Uso: zzmat $funcao $2 base altura"
							return 1
						fi
						;;
					retangulo | losango)
						...
						;;
					trapezio)
						if zztestar numero_real $3 && zztestar numero_real $4 && zztestar numero_real $5; then
							num1=$(echo "$3" | tr ',' '.')
							num2=$(echo "$4" | tr ',' '.')
							num3=$(echo "$5" | tr ',' '.')
							num="((${num1}+${num2})/2)*${num3}"
						else
							zztool erro " Uso: zzmat $funcao $2 base_maior base_menor altura"
							return 1
						fi
						;;
					toro)
						if zztestar numero_real $3 && zztestar numero_real $4 && test $(zzmat compara_num $3 $4) != "igual"; then
							num1=$(echo "$3" | tr ',' '.')
							num2=$(echo "$4" | tr ',' '.')
							num="4*${pi}^2*${num1}*${num2}"
						else
							zztool erro " Uso: zzmat $funcao $2 raio1 raio2"
							return 1
						fi
						;;
					tetraedro | cubo | octaedro | dodecaedro | icosaedro | quadrado | circulo | esfera | cuboctaedro | rombicuboctaedro | rombicosidodecaedro | icosidodecaedro)
						if test -n "$3"; then
							if zztestar numero_real $3; then
								num1=$(echo "$3" | tr ',' '.')
								case $2 in
							...
							elif test 'truncado' = $3 && zztestar numero_real $4; then
								num1=$(echo "$4" | tr ',' '.')
								case $2 in
							...
							elif test 'snub' = $3 && zztestar numero_real $4; then
								num1=$(echo "$4" | tr ',' '.')
								case $2 in
								...
								zztool erro " Uso: zzmat $funcao $2 lado|raio"
								return 1
							fi
						else
							zztool erro " Uso: zzmat $funcao $2 lado|raio"
				...
				case "$2" in
					paralelepipedo)
						if zztestar numero_real $3 && zztestar numero_real $4 && zztestar numero_real $5; then
							...
							zztool erro " Uso: zzmat $funcao $2 comprimento largura altura"
							return 1
						fi
						;;
					cilindro)
						if zztestar numero_real $3 && zztestar numero_real $4; then
							...
							zztool erro " Uso: zzmat $funcao $2 raio altura"
							return 1
						fi
						;;
					cone)
					...
					prisma)
						if zztestar numero_real $3 && zztestar numero_real $4; then
							...
							zztool erro " Uso: zzmat $funcao $2 area_base altura"
							return 1
						fi
						;;
					piramide)
		...
		mmc | mdc)
			if test $# -ge "3"; then
				...
				shift
				for num2 in $*; do
					if zztestar numero $num1 && zztestar numero $num2; then
						test "$num1" -gt "$num2" && num_maior=$num1 || num_maior=$num2
						test "$num1" -lt "$num2" && num_menor=$num1 || num_menor=$num2

						while test "$num_menor" -ne "0"; do
		...
		somatoria | produtoria)
			...
			if test $# -eq "4"; then
			...
			elif test $# -eq "5" && zztestar numero_real $2 && zztestar numero_real $3 && zztestar numero_real $4 && zztool grep_var "x" $5; then
				local equacao numero operacao sequencia num1 num2
				equacao=$(echo "$5" | sed 's/\[/(/g;s/\]/)/g')
				test "$funcao" = "somatoria" && operacao='+' || operacao='*'
				if test $(zzmat compara_num $2 $3) = 'maior'; then
					num1=$2
					num2=$3
				else
					num1=$3
				...
				num=$(for numero in $sequencia; do
		...
		media | soma | produto)
			if test $# -ge "2"; then
				...
				shift
				while test $# -ne "0"; do
					if zztool grep_var "[" "$1" && zztool grep_var "]" "$1"; then
						valor=$(echo "$1" | sed 's/\([0-9]\{1,\}\)\[.*/\1/' | tr ',' '.')
						peso=$(echo "$1" | sed 's/.*\[//;s/\]//')
						if zztestar numero_real "$valor" && zztestar numero "$peso"; then
							if test $funcao = 'produto'; then
					...
					elif zztestar numero_real "$1"; then
						if test $funcao = 'produto'; then
			...
			;;
		fat)
			if test $# -eq "2" -o $# -eq "3" && zztestar numero "$2" && test "$2" -ge "1"; then
				local num1 num2
				if test 's' = "$3"; then
					num1=$(zzseq $2)
				else
		...
		arranjo | combinacao | arranjo_r | combinacao_r)
			if test $# -eq "3" && zztestar numero "$2" && zztestar numero "$3" && test "$2" -ge "$3" && test "$3" -ge "1"; then
				local n p dnp
				n=$(zzmat fat $2)
				p=$(zzmat fat $3)
				dnp=$(zzmat fat $(($2 - $3)))
		...
		newton | binomio_newton)
			if test "$#" -ge "2"; then
				...
				echo "($num1)^$grau"
				for parcela in $(zzseq $((grau - 1))); do
			...
			;;
		pa | pa2 | pg)
			if test $# -eq "4" && zztestar numero_real "$2" && zztestar numero_real "$3" && zztestar numero "$4"; then
				local num_inicial razao passo valor
				num_inicial=$(echo "$2" | tr ',' '.')
				razao=$(echo "$3" | tr ',' '.')
				passo=0
				valor=$num_inicial
				while (test $passo -lt $4); do
					if test "$funcao" = "pa"; then
						valor=$(echo "$num_inicial + ($razao * $passo)" | bc -l |
							awk '{printf "%.'${precisao}'f\n", $1}')
					elif test "$funcao" = "pa2"; then
		...
		r3)
			...
			if test -n "$1"; then
				...
				while test -n "$1"; do
					...
					if (zztestar numero_real ${num%/*} || test ${num%/*} = 'x') && (zztestar numero_real ${num#*/} || test ${num#*/} = 'x'); then
						num3=$((num3 + 1))
						if test $((num3 % 2)) -eq $ind; then
				...
				unset num
				if test $num4 -eq 1 && test -n "$num1" && test -n "$num2"; then
		...
		eq2g)
			...
			if test $# = "4" && zztestar numero_real $2 && zztestar numero_real $3 && zztestar numero_real $4; then
			...
			;;
		d2p)
			if test $# = "3" && zztool grep_var "," "$2" && zztool grep_var "," "$3"; then
				...
				y2=$(echo "$3" | cut -f2 -d,)
				z2=$(echo "$3" | cut -f3 -d,)
				if zztestar numero_real $x1 && zztestar numero_real $y1 && zztestar numero_real $x2 && zztestar numero_real $y2; then
					a=$(echo "(($y1)-($y2))^2" | bc -l)
					b=$(echo "(($x1)-($x2))^2" | bc -l)
					if zztestar numero_real $z1 && zztestar numero_real $z2; then
		...
		vetor)
			if test $# -ge "3"; then
				...
				while (test $# -ge "1"); do
					...
					if test -n "$fi" && zztestar numero_real $valor; then
						...
						elif zztestar numero_real $num1; then
							z1="$num1"
						fi
					fi

					if test -n "$teta" && zztestar numero_real $valor; then
					...
					if zztestar numero_real $valor; then
		...
		egr | err)
			...
			if test $# = "3" && zztool grep_var "," "$2" && zztool grep_var "," "$3"; then
				...
				if zztestar numero_real $x1 && zztestar numero_real $y1 && zztestar numero_real $x2 && zztestar numero_real $y2; then
					...
					if zztestar numero_sinal $a && zztestar numero_sinal $b && zztestar numero_sinal $c; then
		...
		egc)
			...
			if test $# = "3" && zztool grep_var "," "$2"; then
				local a b r A B C
				if zztool grep_var "," "$3"; then
					r=$(zzmat d2p $2 $3)
				elif zztestar numero_real "$3"; then
					r=$(echo "$3" | tr ',' '.')
				else
		...
		egc3p)
			...
			if test $# = "4" && zztool grep_var "," "$2" && zztool grep_var "," "$3" && zztool grep_var "," "$4"; then
				...
				if test $(zzmat det $x1 $y1 1 $x2 $y2 1 $x3 $y3 1) -eq 0; then
					zztool erro "Pontos formam uma reta."
					return 1
				fi

				if ! zztestar numero_real $x1 || ! zztestar numero_real $x2 || ! zztestar numero_real $x3; then
					zztool erro " Uso: zzmat $funcao ponto(a,b) ponto(c,d) ponto(x,y)"
					return 1
				fi

				if ! zztestar numero_real $y1 || ! zztestar numero_real $y2 || ! zztestar numero_real $y3; then
					zztool erro " Uso: zzmat $funcao ponto(a,b) ponto(c,d) ponto(x,y)"
					return 1
				fi
		...
		ege)
			...
			if test $# = "3" && zztool grep_var "," "$2"; then
				...
				if ! zztestar numero_real $a || ! zztestar numero_real $b || ! zztestar numero_real $c; then
					zztool erro " Uso: zzmat $funcao centro(a,b,c) (numero|ponto(x,y,z))"
					return 1
				fi
				A=$(awk 'BEGIN {valor=-2*('$a'); print (valor<0?"":"+") valor}')
		...
		aleatorio | random)
			...
			elif zztestar numero_real $2; then
				max=$(echo "$2" | tr ',' '.')
			fi

			if test $(zzmat compara_num $max $min) = "menor"; then
				n_temp=$max
				max=$min
				min=$n_temp
				unset n_temp
		...
		det)
			...
			if test $# -ge "5" && test $# -le "10"; then
				local num
				shift
				for num in $*; do
					if ! zztestar numero_real "$num"; then
