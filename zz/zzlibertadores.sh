...
zzlibertadores()
{
	...
	case "$1" in
		...
		2 | grupos | segunda)
			for grupo in 1 2 3 4 5 6 7 8; do
	...
	if test '-g' = "$1" && zztool testa_numero $2 && test $2 -le 8 -a $2 -ge 1; then
		echo "Grupo $2"
	...
	if test '-c' = "$1" -o '-cg' = "$1" -o '-gc' = "$1"; then
		if zztool testa_numero $2 && test $2 -le 8 -a $2 -ge 1; then
			grupo="$2"
		...
		if test $ZZCOR -eq 1; then
			test '-n' != "$3" && {
				echo ""
				zzecho -f verde -l preto " Oitavas de Final "
			}
		fi
	fi
}
