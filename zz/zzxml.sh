...
&/g
	...
	if test -n "$notag"; then
		echo 'BEGIN { notag=0 } {' >$cache_notag
		for ntag in $notag; do
			echo '
				if ($0 ~ /<'$ntag' [^>]*[^\/>]>/) { notag++ }
	...
	if test -n "$semtag"; then
		for ntag in $semtag; do
