...
zzplay()
{
	...
	if test -n "$player"; then
		...
		if test "$player" = "mplayer" -a $lista -eq 1; then
		...
		elif test "$player" = "cvlc" -a $lista -eq 1; then
		...
		elif test "$player" = "avplay" -o "$player" = "ffplay"; then
