...
zzclassnum()
{
	...
	test -n "$1" && zztestar numero "$1" || {
		zztool -e uso classnum
		return 1
	}
	...
	if test $(zzmat compara_num $1 0) = 'maior'; then
		...
		if test -n "$num"; then
			...
			if test $? -eq 0; then
				...
				num=$(echo "$1 * 3 - 1" | bc)
				exp=$(zzmat -p15 log $num 2)
				if zztestar numero "$exp" && test "$exp" -ge 2; then
