...
zzfatorar()
{
	...
	if zztool testa_numero "$1" && test $1 -ge 2; then
		...
		if which factor >/dev/null 2>&1; then
		...
		while test ${num_atual} -gt ${primo_atual} -a ${linha_atual} -le $(zztool num_linhas "$cache"); do
			...
			if test "${num_atual}" != "1"; then
				linha_atual=$((${linha_atual} + 1))
				primo_atual=$(sed -n "${linha_atual}p" "$cache")
				test ${#primo_atual} -eq 0 && {
					zztool erro "Valor não fatorável nessa configuração do script!"
					return 1
				}
			fi
		done
