...
8:septilionésimos"
	...
	if zztool testa_numero "$1" && ! zztool grep_var 'R$' "$prefixo"; then
		...
		if test $qtde_p -eq 1 -a $qtde_v -eq 0 && zztestar numero_fracionario "$1"; then
			if echo "$1" | grep '^[0-9]\{1,3\}\.[0-9]\{3\}$' >/dev/null; then
				numero=$(echo "$1" | tr -d '.')
			else
				numero=$(echo "$1" | tr '.' ',')
			fi
		...
		if test $qtde_v -eq 1 -a $qtde_p -eq 0 && zztestar numero_fracionario "$1"; then
			numero="$1"
		fi

		# Número com o "ponto" como separador de milhar, e sem parte fracionária
		if (test $qtde_p -gt 1 -a $qtde_v -eq 0 && test -z $numero); then
		...
		if test -n $numero; then
			...
			if test ${#num_frac} -gt 0; then
				zztool testa_numero $num_frac || {
					zztool -e uso numero
					return 1
				}
			fi
			...
			if test "$prec" != "-" && test $prec -ge 0 && test ${#n_formato} -eq 0; then
				...
				if test ${#num_frac} -gt $prec; then
					...
					if test $(echo "$num_frac" | cut -c $((prec + 1))) -ge 6; then
						...
						if test $prec -eq 0; then
							...
							if echo "$num_frac" | grep -E '^9{1,}$' >/dev/null; then
								num_int=$(echo "$num_int + 1" | bc)
								num_frac=0
							else
					...
					elif test $(echo "$num_frac" | cut -c $((prec + 1))) -le 4; then
						...
						if test $prec -eq 0; then
							...
							if test $(($n_temp % 2)) -eq 1; then
								num_int=$(echo "$num_int + 1" | bc)
							fi
						else
							# Determinando último número dentro da precisão é par
							if test $(echo $(($(echo $num_frac | cut -c ${prec}) % 2))) -eq 0; then
					...
					while test ${#num_frac} -lt $prec -a ${#num_frac} -gt 0; do
						num_frac="0${num_frac}"
					done
				fi
			...
			if zztool grep_var 'R$' "$prefixo"; then
	...
	if test $texto -eq 1 -o $texto -eq 2; then
				...
				if test $texto -eq 2; then
					...
					case ${#qtde_v} in
						1)
							...
							;;
						2)
							if test $(echo "$qtde_v" | cut -c1) -eq 1; then
								# Entre 10 e 19, captura direta do texto no segundo campo
								numero=$(echo "$ordem1" | grep "^${qtde_v}:" | cut -f2 -d":")
							elif test $(echo "$qtde_v" | cut -c2) -eq 0; then
						...
						3)
							if test $qtde_v -eq 100; then
								...
								# Dezena
								n_temp=$(echo "$qtde_v" | cut -c2)
								if test "$n_temp" != "0"; then
									if test "$n_temp" = "1"; then
		...
		if zztool grep_var ' mil' "${num_saida}"; then
			# Colocando o "e" entre o mil seguido de 1 ao 19
			for n_temp in $(echo "$ordem1" | cut -f 2 -d: | sed '/^ *$/d'); do
			...
			for n_temp in $(echo "$ordem1" | cut -f 3 -d: | sed '/^ *$/d'); do
				num_saida=$(echo "${num_saida}" | sed 's/^ *//;s/ *$//' | sed "s/ mil $n_temp$/ mil e $n_temp/")
				num_saida=$(echo "${num_saida}" | sed 's/^ *//;s/ *$//' | sed "s/^ *mil $n_temp$/ mil e $n_temp/")
			done
			...
			for milhar_para in $(echo "$ordem1" | sed -n '3,10p' | cut -f3 -d:); do
				for decimal_para in $(echo "$ordem1" | sed -n '2,10p' | cut -f2 -d:); do
		...
		if test "$n_temp" != "0" -a "$n_temp" != "zero"; then
		...
		if test "$num_int" = "1"; then
			if test ${#num_frac} -eq 0; then
				num_saida=$(echo $num_saida | sed 's/s$//')
			elif test "$num_frac" = "00"; then
				num_saida=$(echo $num_saida | sed 's/s$//')
			fi
		fi
...
/')
		...
		if test "$n_temp" = "0" -o "$n_temp" = "R$ 0"; then
			num_saida=$n_temp
		fi
