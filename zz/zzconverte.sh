...
zzconverte()
{
	...
	while test -n "$1"; do
		...
		case "$operacao" in
			c[dh]) echo "$1" | grep '^-' >/dev/null && {
				shift
				continue
			} ;;
			b[dho]) zztestar binario "$1" || {
				shift
				continue
			} ;;
			d[bohc]) zztestar numero "$1" || {
				shift
				continue
			} ;;
			o[bdh]) zztestar octal "$1" || {
				shift
				continue
			} ;;
			h[bdoc])
		...
		case "$operacao" in
			...
			fk)
				suf1="°F"
				suf2="K"
				bc_expr="$s2;($1+459.67)/1.8"
				;;
			kf)
				suf1="K"
				suf2="°F"
				bc_expr="$s2;($1*1.8)-459.67"
				;;
			# Distância:
			km)
				suf1="km"
			...
			oh)
				suf1="em octal"
				suf2="em hexadecimal"
				bc_expr="obase=16;ibase=8;${1#0}"
				;;
			# Hexadecimal:
			hb)
				suf1="em hexadecimal"
				suf2="em binário"
				bc_expr="obase=2;ibase=16;$num_hex"
				;;
			ho)
				suf1="em hexadecimal"
				suf2="em octal"
				bc_expr="obase=8;ibase=16;$num_hex"
				;;
			hd)
				suf1="em hexadecimal"
				suf2="em decimal"
			...
			dc | hc | oc)
				case "$operacao" in
					...
					oc)
						suf1="em octal"
						fator=$(echo "ibase=8;${1#0}" | bc)
						;;
				esac
				suf2="em caractere"

				if test "$fator" -ge 32 -a "$fator" -le 126; then
					octal=$(printf "%03o\n" "$fator")
					resp=$(printf "\\$octal")
				elif test "$fator" -ge 161 -a "$fator" -le 191; then
					octal=$(echo "obase=8;$fator" | bc)
					resp=$(printf "\302\\$octal")
				elif test "$fator" -ge 192 -a "$fator" -le 255; then
			...
			gr)
				suf1="°"
				suf2="rad"
				resp=$(echo "$s2;$pi;$1*pi/180" | bc -l | zznumero --para en | tr -d ,)
				;;
			rg)
				suf1="rad"
				suf2="°"
				resp=$(echo "$s2;$pi;$1*180/pi" | bc -l | zznumero --para en | tr -d ,)
				;;
			ga)
				suf1="°"
				suf2="gon"
				resp=$(echo "$s2;$1/0.9" | bc -l | zznumero --para en | tr -d ,)
				;;
			ag)
				suf1="gon"
				suf2="°"
			...
			ra)
				suf1="rad"
				suf2="gon"
				resp=$(echo "$s2;$pi;$1*200/pi" | bc -l | zznumero --para en | tr -d ,)
				;;
			ar)
				suf1="gon"
				suf2="rad"
				resp=$(echo "$s2;$pi;$1*pi/200" | bc -l | zznumero --para en | tr -d ,)
				;;
			*)
				zztool erro "Conversão inválida $operacao"
				return 1
				;;
		esac
