...
zztop()
{
	...
	while test -n "$1"; do
		...
		case "$1" in
			vendor | app | accel | procgen | os)
				category="$1"
				ntab=35
				shift
				;;
			connfam | interconnect | country | region | accelfam)
				category="$1"
				ntab=28
				shift
				;;
			segment | continent | architecture | osfam | cores)
				category="$1"
				ntab=12
				shift
				;;
	...
	if test -z "$release"; then
		release=$max_release
	elif test "$release" -gt "$max_release"; then
		release=$max_release
	fi
