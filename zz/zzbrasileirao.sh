...
zzbrasileirao()
{
	...
	test $(date +%Y%m%d) -lt 20190426 && {
		zztool erro "Campeonato Brasileiro 2018 só a partir de 14 de Abril."
		return 1
	}
	...
	if test -n "$1"; then
		zztool testa_numero "$1" && rodada="$1" || {
			zztool -e uso brasileirao
			return 1
		}
	fi
	...
	if test -n "$rodada"; then
		case $serie in
			a | b | c)
				printf "$(
					...
					sed 's/"//g;s/null/ /g' |
					zztool nl_eof |
					while IFS="_" read time1 placar time2; do
		...
		zztool source "$url" |
			...
			zzxml --untag |
			sed 's/| *$//' |
			while IFS='|' read pos time resto; do
				case "$pos" in
					"GRUPO A") printf "$pos\n" ;;
